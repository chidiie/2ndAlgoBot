...
public sealed class BacktestEngine : IBacktestEngine
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        Console.WriteLine($"""
                ---------------- BACKTEST SUMMARY ----------------
                Period:         {summary.StartUtc:yyyy-MM-dd} to {summary.EndUtc:yyyy-MM-dd}
                Balance:        {summary.StartingBalance:C} --> {summary.EndingBalance:C}
                Net Profit:     {summary.NetProfit:C} 
                Net Profit:     {summary.NetProfit:C} ({((summary.EndingBalance / summary.StartingBalance) - 1) * 100:F2}%)
                Starting Balance{summary.StartingBalance}  
                Ending Balance  {summary.EndingBalance}

                Trades:         {summary.TotalTrades} (Wins: {summary.Wins}, Losses: {summary.Losses}, BE: {summary.Breakevens})
                Win Rate:       {summary.WinRatePercent:F2}%
                Profit Factor:  {summary.ProfitFactor:F2}
                Expectancy:     {summary.ExpectancyPerTrade:C} / trade

                Avg Win:        {summary.AverageWin:C}
                Avg Loss:       {summary.AverageLoss:C}
                Gross Profit:   {summary.GrossProfit:C}
                Gross Loss:     {summary.GrossLoss:C}

                Max Drawdown:   {summary.MaxDrawdownAmount:C} ({summary.MaxDrawdownPercent:F2}%)
                --------------------------------------------------
                """);
    ...
    CancellationToken cancellationToken)
    {
        ...
        //string setup = $"M30range_M5bk_M1entry";
        ...
        //for (var day = localEnd; day >= localStart; day = day.AddDays(-1))
        {
            ...
            //if (window.SessionEndUtc > endUtc || window.SessionStartUtc < startUtc)
            //    continue;
    ...
    IReadOnlyDictionary<string, List<Candle>> indicatorCandlesByTimeframe)
    {
        ...
        var effectiveBreakoutTimeUtc = ResolveEffectiveBreakoutTimeUtc(
    breakout.Candle,
    breakout.Direction,
    range,
    sessionEntryCandles,
    strategy.BreakoutTimeframe,
    entryTimeframe,
    allowHybridLowerTimeframeBreakout: true);

        //var candlesAfterBreakout = useSeparateEntryTimeframe
        //    ? sessionEntryCandles
        //        .Where(c => c.Time >= effectiveBreakoutTimeUtc)
        //        .OrderBy(c => c.Time)
        //        .ToList()
        //    : postOrbCandles
        ...
        //var breakoutCandleCloseTime = breakout.TimeUtc + TradingTimeHelper.GetTimeframeSpan(strategy.BreakoutTimeframe);


        //var candlesAfterBreakout = useSeparateEntryTimeframe
        //    ? sessionEntryCandles
        //        .Where(c => c.Time >= breakoutCandleCloseTime)
        //        .OrderBy(c => c.Time)
        //        .ToList()
        //    : postOrbCandles
        //        .Where(c => c.Time >= breakout.TimeUtc)
        ...
        var candlesForFib = useSeparateEntryTimeframe
    ? sessionEntryCandles
        .Where(c => c.Time > breakout.TimeUtc)
        .OrderBy(c => c.Time)
        .ToList()
    : postOrbCandles
        ...
        Candle? confirmationCandle = null;

        if (RequiresRetest(strategy.EntryMode))
        {
            confirmationCandle = ConfirmRetest(breakout.Direction, range, candlesAfterBreakout, spec, strategy.Retest);
            if (confirmationCandle is null)
                return null;
        }

        if (RequiresFib(strategy.EntryMode))
        {
            var fibCandle = ConfirmFib(breakout.Direction, range, candlesAfterBreakout, spec, strategy.Fibonacci);
            if (fibCandle is null)
                return null;

            // take the later of retest and fib confirmations if both required
            confirmationCandle = confirmationCandle is null
                ? fibCandle
                : (fibCandle.Time > confirmationCandle.Time ? fibCandle : confirmationCandle);
        }
        ...
        var entryBar = backtest.UseNextBarOpenForEntry
    ? candlesAfterBreakout.FirstOrDefault(c => c.Time > (confirmationCandle?.Time ?? breakout.TimeUtc))
    : confirmationCandle ?? breakout.Candle;

        if (entryBar is null)
            return null;

        var rawEntry = backtest.UseNextBarOpenForEntry ? entryBar.Open : entryBar.Close;
        ...
        var qty = SizeVolumeForBacktest(spec, quote, instrument, stopDistancePrice, riskAmount);
        ...
        var profitLoss = (priceMove / spec.TickSize) * tickValue.Value * qty;
    ...
    private DateTime ResolveEffectiveBreakoutTimeUtc(
    Candle breakoutCandle,
    TradeDirection breakoutDirection,
    OrbRange range,
    IReadOnlyList<Candle> sessionEntryCandles,
    string breakoutTimeframe,
    string entryTimeframe,
    bool allowHybridLowerTimeframeBreakout)
    {
        var breakoutSpan = TradingTimeHelper.GetTimeframeSpan(breakoutTimeframe);
        var entrySpan = TradingTimeHelper.GetTimeframeSpan(entryTimeframe);

        var breakoutCloseTimeUtc = breakoutCandle.Time + breakoutSpan;

        if (!allowHybridLowerTimeframeBreakout ||
            string.Equals(breakoutTimeframe, entryTimeframe, StringComparison.OrdinalIgnoreCase))
        {
            return breakoutCloseTimeUtc;
        }

        var lowerTfCandlesInsideBreakoutBar = sessionEntryCandles
            .Where(c => c.Time >= breakoutCandle.Time && c.Time < breakoutCloseTimeUtc)
            .OrderBy(c => c.Time)
            .ToList();

        foreach (var c in lowerTfCandlesInsideBreakoutBar)
        {
            var lowerTfBreakout = breakoutDirection == TradeDirection.Buy
                ? c.Close > range.High
                : c.Close < range.Low;

            if (lowerTfBreakout)
            {
                // this 1m candle becomes known only when it closes,
                // so the next eligible entry candle starts from its close time
                return c.Time + entrySpan;
            }
        }
    ...
    private static Candle? ConfirmRetest(
    ...
    private static Candle? ConfirmFib(
    TradeDirection direction,
    OrbRange range,
    IReadOnlyList<Candle> candlesAfterBreakout,
    SymbolSpecification spec,
    FibonacciSettings settings)
    {
        ...
        if (direction == TradeDirection.Buy)
        {
            ...
            l382 = impulseHigh - diff * 0.382m;
            l500 = impulseHigh - diff * 0.500m;
            l618 = impulseHigh - diff * 0.618m;
            l786 = impulseHigh - diff * 0.786m;
        }
        else
        {
            ...
            l382 = impulseLow + diff * 0.382m;
            l500 = impulseLow + diff * 0.500m;
            l618 = impulseLow + diff * 0.618m;
            l786 = impulseLow + diff * 0.786m;
        }

        levels = GetFibLevels(settings, l382, l500, l618, l786);
        if (levels.Count == 0)
            return null;
        ...
        {
            return subset.FirstOrDefault(c =>
                PriceHelper.CandleTouchesZone(c, zoneLower, zoneUpper, tolerance));
        }
        else
        {
            return subset.FirstOrDefault(c =>
                (settings.UseLevel0382 && PriceHelper.CandleTouchesLevel(c, l382, tolerance)) ||
                (settings.UseLevel0500 && PriceHelper.CandleTouchesLevel(c, l500, tolerance)) ||
                (settings.UseLevel0618 && PriceHelper.CandleTouchesLevel(c, l618, tolerance)) ||
                (settings.UseLevel0786 && PriceHelper.CandleTouchesLevel(c, l786, tolerance)));
        }
    }
    ...
    IReadOnlyDictionary<string, List<Candle>> indicatorCandlesByTimeframe)
    {
        var breakoutCandleCloseTime = breakoutTimeUtc +
        TradingTimeHelper.GetTimeframeSpan(strategy.BreakoutTimeframe);
    ...
    decimal riskAmount)
    {
        if (stopDistancePrice <= 0 || riskAmount <= 0)
            return 0m;
        var contractSize = ResolveContractSize(spec, instrument);
        var tickValue = quote.LossTickValue;
        ...
        var lossPerLot = (stopDistancePrice / spec.TickSize) * tickValue;
        //var lossPerLot = stopDistancePrice * contractSize;
        if (lossPerLot <= 0)
            return 0m;

        var rawVolume = riskAmount / lossPerLot.Value;
        if (rawVolume <= 0)
            return 0m;
    ...
    private BacktestExit SimulateExit(
    TradeDirection direction,
    ...
    {
        var trailing = _botSettings.CurrentValue.Risk.TrailingStop;
        var sessionExit = _botSettings.CurrentValue.Risk.SessionExit;

        var exitAdj = PriceHelper.PipsToPriceDistance(spec, exitSpreadPips);
        var currentStop = initialStopLoss;
        var initialRisk = Math.Abs(entryPrice - initialStopLoss);

        foreach (var bar in futureBars)
        {
            // Conservative candle-based order:
            // 1) check current SL/TP first
            // 2) only after the bar completes, update trailing stop for the next bar

            if (direction == TradeDirection.Buy)
            {
                var hitSl = bar.Low <= currentStop;
                var hitTp = bar.High >= takeProfit;

                if (hitSl && hitTp)
                    return new BacktestExit(currentStop - exitAdj, bar.Time, "SameBar_SL_First");

                if (hitSl)
                    return new BacktestExit(currentStop - exitAdj, bar.Time, "StopLoss");

                if (hitTp)
                    return new BacktestExit(takeProfit - exitAdj, bar.Time, "TakeProfit");

                if (trailing.Enabled && initialRisk > 0)
                {
                    var moved = bar.High - entryPrice;
                    var activationDistance = initialRisk * trailing.ActivationR;

                    if (moved >= activationDistance)
                    {
                        var trailDistance = initialRisk * trailing.DistanceR;
                        var lockPrice = entryPrice + (initialRisk * trailing.LockInR);
                        var candidate = bar.High - trailDistance;
            ...
            {
                var hitSl = bar.High >= currentStop;
                ...
                if (trailing.Enabled && initialRisk > 0)
                {
                    ...
                    if (moved >= activationDistance)
                    {
                        var trailDistance = initialRisk * trailing.DistanceR;
                        var lockPrice = entryPrice - (initialRisk * trailing.LockInR);
                        var candidate = bar.Low + trailDistance;
        ...
        // Configurable session-end fallback
        if (sessionExit.Enabled)
        {
            var last = futureBars[^1];
            return new BacktestExit(last.Close, last.Time, "SessionEnd");
        }

        var finalBar = futureBars[^1];
        return new BacktestExit(finalBar.Close, finalBar.Time, "DataEnd");
    }
